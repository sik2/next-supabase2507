...
export default function EditPostClient({ postId }: EditPostClientProps) {
  ...
  useEffect(() => {
    ...
    const existingPost = postService.getPostById(postId);
    if (!existingPost) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      postService.updatePost(post.id, updates);
      router.push('/dashboard');
    } catch (err: any) {
    ...
    return (
      <div className="min-h-screen bg-gray-50 flex items-center justify-center">
        <div className="text-center">
          ...
          <Link href="/dashboard" className="text-blue-600 hover:text-blue-700 cursor-pointer">
            대시보드로 돌아가기
          </Link>
        </div>
      </div>
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="bg-white rounded-lg shadow-sm border">
          <div className="p-6 border-b border-gray-200">
            <h1 className="text-2xl font-bold text-gray-900">공부기록 수정</h1>
            <p className="text-gray-600 mt-1">포스트 내용을 수정해보세요</p>
          </div>
